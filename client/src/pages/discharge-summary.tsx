      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-4 gap-4 border p-4 rounded-lg">
          {timelineEvents.filter(e => e.type === "vital").slice(0, 1).map(v => (
             <>
               <div key="t" className="text-center"><p className="text-[10px] uppercase">Temp</p><p className="font-bold">{v.description.split(",")[0].split(":")[1]}</p></div>
               <div key="bp" className="text-center"><p className="text-[10px] uppercase">BP</p><p className="font-bold">{v.description.split(",")[1].split(":")[1]}</p></div>
               <div key="sp" className="text-center"><p className="text-[10px] uppercase">SpO2</p><p className="font-bold">{v.metadata.SpO2}</p></div>
               <div key="hr" className="text-center"><p className="text-[10px] uppercase">HR</p><p className="font-bold">{v.metadata.HR}</p></div>
             </>
          ))}
        </div>
      </section>
